...
その場合はstaticを使わずにconstで設定する
※定数は慣習的に大文字にするのと$マークはいらないので注意
...
# クラスの継承について
### extends Postを使うとクラスを継承することができ､継承したプロパティやメソッドを使うことができる
...
```
# 子クラスでの独自の実装について
### クラスを継承するとそのクラス独自のコードだけを書いていけば良い
```php
...
# メソッドのオーバーライドについて
### overrideとは親クラスと同名のメソッドを再定義すること
```php
...
# 型の継承について
### クラスの継承はプロパティやメソッドだけでなくデータ型も継承することが可能｡将来的にクラスを継承したクラスをたくさん作ってもコードの変更をまとめて処理できるというメリットがある｡
```php
<?php
...
abstractキーワードを使って作成する｡定義自体は子クラスで行うので中身は書かなくてもよい  
abstarctは最初に書くことが多く､abstractをつけたメソッドのことを抽象メソッドと呼ぶ

# 抽象メソッドについて
抽象クラスはある程度開発規模が大きくなってから使うもので､子クラスの方で定義を強制したい時に便利
```php
<?php
...
# クラスの拡張について
クラスを増やしても､抽象クラスや型の継承を行うと定義漏れを防ぎつつ､共通の方でまとめることができる｡

- 例　新しく有料投稿を実装したい場合
形がsponsoredPostと同じような形になるので､コピーして使うことにする
```php
<?php
